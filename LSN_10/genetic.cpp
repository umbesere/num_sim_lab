...
// to create the square map
vector<vector<double>> Square( int ncities, Random &rnd ){
    vector<vector<double>> v(ncities, vector<double>(2,0));
...
//to generate the starting population
vector<vector<int>> StartingPop( int pop_size, int ncities, Random &rnd){
    vector<vector<int>> v(pop_size, vector<int>(ncities,0));
...
//to generate the starting individual
vector<int> StartingInd( int ncities, Random &rnd){
    vector<int> v( ncities, 0 );
    for(int j=0; j<ncities; j++)
        v[j] = j;
    for(int i=1; i<100; i++)
        v = Mutation_swap(v,rnd);
    return v;
};
...
//to compute the length of an individual
