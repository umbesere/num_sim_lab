...
//g(r)
    for(int i=igofr; i<n_props;i++){
      deltaV = 4./3.* pi * ( pow((i-1)*bin_size,3) - pow((i-2)*bin_size,3) );
      stima_gdir = blk_av[i]/blk_norm / (rho * m_part * deltaV);
    ...
    if (iblk==nblk){
      for(int i=igofr; i<n_props;i++){
        ...
        Gave << setw(wd) << (i-igofr+1./2.)*bin_size << setw(wd) << glob_av[i] / nblk << setw(wd) << err_gdir << endl;
      }
    }
...
//printing instatanuoeus values
void InstaPrint(int istep){

  int wd = 16;
  ofstream InstaMes("instames.txt", ios::app);
  InstaMes << setw(wd) << istep << setw(wd) << walker[iv]/(double) npart + vtail << setw(wd) << rho * temp + (walker[iw] + ptail * (double)npart) / vol << endl;
  InstaMes.close();
};
