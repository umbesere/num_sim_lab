...
int main()
{
	...
	for (int iblock = 1; iblock <= nblock; iblock++){
		...
		Averages(iblock);   //Print results for current block	
	}
...
{ //Prepare all stuff for the simulation
	...
	double sumv2 = 0.0, fs;
	if (!restart) // for the first run of the algorithm 
	{
...
double Error(double ave, double av2, int n)
{
	if (n == 1)
		...
		return sqrt((av2/(double)n - pow(ave/(double)n, 2)) / (double)(n-1));
}
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    Epot.open("average_epot.out",ios::app);
    Ekin.open("average_ekin.out",ios::app);
    Etot.open("average_etot.out",ios::app);
    Temp.open("average_temp.out",ios::app);
    Gofr.open("average_gofr.out",ios::app);
    Gave.open("average_gave.out",ios::app);
...
//g(r)
    ...
    if (iblk==nblock){
		for (int i = igofr; i < n_props; i++){
      		...
      		Gave << setw(wd) << (i-igofr+1./2.)*bin_size << setw(wd) << glob_av[i] / nblock << setw(wd) << err_gofr << endl;
		}
	}
