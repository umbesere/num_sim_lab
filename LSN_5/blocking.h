...
class UnifMCMC : public BasicBlockMean {

	public:
		UnifMCMC( shared_ptr<MCMC> myMCMC){
			m_MCMC = myMCMC;
		};
		~UnifMCMC(){};					// virtual destructor
		double BlockMean( int L );		// key function
	
	private:
		shared_ptr<MCMC> m_MCMC;
		shared_ptr<Random> m_rnd = make_shared<Random>("seed.out");
};

class GaussMCMC : public BasicBlockMean {

	public:
		GaussMCMC( shared_ptr<MCMC> myMCMC){
			m_MCMC = myMCMC;
		};
		~GaussMCMC(){};					// virtual destructor
		double BlockMean( int L );		// key function
